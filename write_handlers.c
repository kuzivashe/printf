...
int write_num(int ind, char buffer[],
		...
		int length, int padd, char extra_c)
{
	int i, padd_start = 1;
	...
	if (width > length)
	{
		...
		else if (!(flags & F_MINUS) && padd == '0') /* extra char to the left of the padd */
		{
			if (extra_c)
...
int write_unsgnd(int is_negative, int ind, char buffer[],
		int flags, int width, int precision, int size)
{
	/* the number is stored at the buffer's and starts at position i */
		...
		int width, int flags, char padd, char extra_c, int padd_start)
{
	...
	if (width > length)
	{
		for (i = 3; i < width - length + 3; i++)
			buffer[i] = padd;
		buffer[i] = '\0';
		if (flags & F_MINUS && padd == ' ') /* assign extra char to left to buffer */
		{
			buffer[--ind] = 'x';
			buffer[--ind] = '0';
			if (extra_c)
